...
table_output <- results_table %>%
  mutate(
    ...
    `Postal Code 10437` = case_when(
      str_detect(rowname, "Equity") ~ paste0(formatC(100 * as.numeric(`Postal Code 10437`), format = "f", digits = 2, big.mark = ".", decimal.mark = ","), "%"),
      TRUE ~ format(as.numeric(`Postal Code 10437`), big.mark = ".", decimal.mark = ",", scientific = FALSE)
    ),
    `Other Berlin Firms` = case_when(
      str_detect(rowname, "Equity") ~ paste0(formatC(100 * as.numeric(`Other Berlin Firms`), format = "f", digits = 2, big.mark = ".", decimal.mark = ","), "%"),
      TRUE ~ format(as.numeric(`Other Berlin Firms`), big.mark = ".", decimal.mark = ",", scientific = FALSE)
    ),
    Difference = case_when(
      str_detect(rowname, "Equity") ~ paste0(formatC(100 * as.numeric(Difference), format = "f", digits = 2, big.mark = ".", decimal.mark = ","), "%"),
      TRUE ~ format(as.numeric(Difference), big.mark = ".", decimal.mark = ",", scientific = FALSE)
    ),
    `P-Value` = case_when(
      is.na(`P-Value`) ~ "",
      `P-Value` < 0.001 ~ "<0.001",
...
gt_tbl <- table_output %>%
  ...
  tab_source_note(
    source_note = paste0(
      ...
      n_removed_assets, " excluded)."
    )
  )

# Render table to R Markdown
print(gt_tbl)
```

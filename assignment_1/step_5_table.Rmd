...
doc_table <- flextable(results_table %>% select(-Significance)) %>%
  autofit()
...
doc <- read_docx() %>%
  ...
  body_add_par("Note: Total Assets in EUR. Equity Ratio = Equity / Total Assets. P-values from Welch's t-test (for means and SDs) and Wilcoxon rank-sum test (for medians). Significance levels: * p<0.1, ** p<0.05, *** p<0.01. N refers to the number of firms in each group. All p-values <0.001 are reported as <0.001.")
